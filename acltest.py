...
def BL_verify_age(params, issuer_pub, num_attributes, signature, sig, gam_hs, gam_g):
    m = BL_check_signature(params, issuer_pub, signature)
    assert m != False
...
def test_modular():
    ...
    sig, gam_hs, gam_g = BL_user_prove_cred(LT_user_state)

    # Check signature and ZK proof
    #m = BL_verify_cred(params, issuer_pub, 2, signature, sig)
    m = BL_verify_age(params, issuer_pub, 1, signature, sig, gam_hs, gam_g)
    print(m != False)
    assert m != False
